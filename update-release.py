...
Only touches the .TH line in man/*.1, the last line of README, and the
version defines in configure.ac.  Copyright lines are never modified.
"""
...
def update_readme(new_date, dry_run):
...
def update_configure_ac(version_str, dry_run):
    ...
    replacements = {
        r'm4_define\(\[fuse_utils_version\],\s*\[\S+\]\)':
            f'm4_define([fuse_utils_version], [{version_str}])',
        r'm4_define\(\[fuse_utils_major_version\],\s*\[\S+\]\)':
            f'm4_define([fuse_utils_major_version], [{major}])',
        r'm4_define\(\[fuse_utils_minor_version\],\s*\[\S+\]\)':
            f'm4_define([fuse_utils_minor_version], [{minor}])',
        r'm4_define\(\[fuse_utils_micro_version\],\s*\[\S+\]\)':
            f'm4_define([fuse_utils_micro_version], [{micro}])',
        r'm4_define\(\[fuse_utils_nano_version\],\s*\[\S+\]\)':
            f'm4_define([fuse_utils_nano_version],  [{nano}])',
    }
...
def main():
    ...
    if update_readme(args.date, args.dry_run):
        changed += 1
